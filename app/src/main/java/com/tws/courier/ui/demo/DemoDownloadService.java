 ...
 */
package com.tws.courier.ui.demo;
...
public class DemoDownloadService extends DownloadService {
    ...
    public static ArrayList<VideoDownload> videoDownloads = new ArrayList<>();

    public DemoDownloadService() {
        super(
                FOREGROUND_NOTIFICATION_ID,
                DEFAULT_FOREGROUND_NOTIFICATION_UPDATE_INTERVAL,
                CourierApp.DOWNLOAD_NOTIFICATION_CHANNEL_ID,
    ...
    protected DownloadManager getDownloadManager() {
        ...
        CourierApp application = (CourierApp) getApplication();
...
//        notification.actions
        return ((CourierApp) getApplication())
                .getDownloadNotificationHelper()
                .buildProgressNotification(
                        R.mipmap.ic_noti_small_, /* contentIntent= */ null, /* message= */ null, downloads);
    }
    ...
    private static final class TerminalStateNotificationHelper implements DownloadManager.Listener {
        ...
        public void onDownloadChanged(DownloadManager manager, Download download) {
            ...
            if (download.state == Download.STATE_COMPLETED) {
                onDownloadComplete(download.request.uri.toString());
                notification =
        ...
        private void onDownloadComplete(String url) {
            VideoDownload vdio = null;
            for (VideoDownload vd : videoDownloads) {
                if (vd.getVideoUrl().equals(url)) {
                    AppManager.Companion.saveVideoDownload(vd);
                    vdio = vd;
                }
            }
            videoDownloads.remove(vdio);
        }
    }
}
